...
class LocationServer {
    constructor() {
		...
		this.location = {};
		this.loot = {};
		this.rarityMultipliers = {"not_exist": 0, "common": 1, "rare": 0.7, "superrare": 0.4};
    }

    /* Load all the locations into memory. */
    initialize(rarityMultipliers, containerMultipliers) {
		this.locations = fileIO.readParsed(db.user.cache.locations);
		this.loot = fileIO.readParsed(db.cacheBase.location_statics);
        this.rarityMultipliers = rarityMultipliers;
        this.containerMultipliers = containerMultipliers;
    }
    ...
    generate(name) {
		...
		this.location = fileIO.readParsed(db.locations[name]);
	...
	generateContainerLoot(_items) {
		...
		// LootList[_ID_]._props.SpawnChance
		
		// spawn filter need to be done for categories to add indyvidual items to the list
        ...
        let container2D = Array(container._props.Grids[0]._props.cellsV).fill(0).map(() => Array(container._props.Grids[0]._props.cellsH).fill(0));
		...
		for(let item in LootList){
			if(LootList[item]._props.SpawnChance)
				totalChance += LootList[item]._props.SpawnChance * this.getRarityMultiplier(LootList[item]._props.Rarity);
		}
        ...
        for (let i = 0; i < minCount; i++)
        {
            ...
            while (!result.success && maxAttempts)
            {
				...
				for(let lootItem in LootList){
					...
					currentTotal += LootList[lootItem]._props.SpawnChance * this.getRarityMultiplier(LootList[lootItem]._props.Rarity);
				...
				//if(rolling_pool.length > 0)
					//rolled = rolling_pool[Math.floor(Math.random() * rolling_pool.length)];
...
*/
    ...
    generateAll() {
		...
		{
			if(fileIO.exist(db.user.cache.locations))
			{
				let base = fileIO.readParsed(db.cacheBase.locations);
				let data = fileIO.readParsed(db.user.cache.locations);
				let newData = {};
